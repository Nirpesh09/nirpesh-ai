    ...
    <div className="min-h-screen relative overflow-hidden" style={{ background: "#000" }}>
      ...
      <main className="relative z-10 mx-auto max-w-3xl px-6 pt-16 pb-24 text-center">
        ...
        >
          ...
          <div className="flex items-center justify-between px-3 pb-3 gap-2">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs hidden sm:block font-mono" style={{ color: "#4ade80", opacity: 0.6 }}>⏎ to send</span>
              <button
                onClick={() => start()}
        ...
        <div id="ideas" className="mt-6 flex flex-wrap gap-2 justify-center">
          {IDEAS.map((q) => (
            <button key={q} onClick={() => start(q)}
              className="px-3.5 py-1.5 rounded-full border text-sm font-mono transition-all hover:scale-105"
              style={{ background: "rgba(0,255,80,0.05)", borderColor: "rgba(0,255,80,0.20)", color: "rgba(134,239,172,0.85)" }}
              onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "rgba(0,255,80,0.12)"; (e.currentTarget as HTMLButtonElement).style.borderColor = "rgba(0,255,80,0.5)"; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "rgba(0,255,80,0.05)"; (e.currentTarget as HTMLButtonElement).style.borderColor = "rgba(0,255,80,0.20)"; }}
            >
              {q}
            </button>
          ))}
      ...
      <section id="apps" className="relative z-10 mx-auto max-w-6xl px-6 pb-24">
        <div
          ...
          style={{ background: "rgba(0,5,0,0.80)", borderColor: "rgba(0,255,80,0.15)", backdropFilter: "blur(16px)" }}
        >
          ...
          {apps.length === 0 ? (
            <button
              onClick={() => { if (!authUser) { navigate({ to: "/login", search: { next: "/" } as never }); return; } document.querySelector("textarea")?.focus(); }}
